...
CAN_PY="$WS/src/mobile/read_velocity/can_communicate_v4.py"
...
TIMEOUT_CAMERA_MGR=15       # có tác dụng khi dùng web
TIMEOUT_MAP_EDITOR=15       # có tác dụng khi dùng web
# =======================
...
# -----------------------
# Window: CAMERA MANAGER  ← WEB
# -----------------------
tmux new-window -t "$SESSION" -n "camera_mgr"
tmux send-keys -t "$SESSION:camera_mgr" \
  "tmux wait-for STAGE_CAMERA_MGR; \
   cd \"$WS\" && source install/setup.bash && \
   tmux wait-for -S CAMERA_MGR_OK; \
   python3 \"$WS/src/mobile/read_velocity/camera_manager_node.py\"" C-m

# Thêm vào cuối script SLAM, sau khi SLAM đã start xong
tmux new-window -t "$SESSION" -n "map_editor"
tmux send-keys -t "$SESSION:map_editor" \
  "tmux wait-for STAGE_MAP_EDITOR; \
   cd \"$WS\" && source install/setup.bash && \
   tmux wait-for -S MAP_EDITOR_OK; \
   python3 \"$WS/src/mobile/read_velocity/map_editor_node.py\"" C-m


# =======================
# Orchestrate order
# =======================
log "[3/4] Run order: IMU -> LiDAR -> CAN -> SLAM"
...
tmux wait-for -S STAGE_CAMERA_MGR                                               # web
timeout "${TIMEOUT_CAMERA_MGR}s" tmux wait-for CAMERA_MGR_OK || die "CAMERA_MGR stage timed out"  # ← web

tmux wait-for -S STAGE_MAP_EDITOR                                               # web
timeout "${TIMEOUT_MAP_EDITOR}s" tmux wait-for MAP_EDITOR_OK || die "MAP_EDITOR stage timed out" 
