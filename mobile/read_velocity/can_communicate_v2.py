...
def main():
    ...
    try:
        while True:
            ...
            # ===== NEW: READ REAL SPEED =====
            spL = read_speed_rpm(ser, NODE_L)
            spR = read_speed_rpm(ser, NODE_R)
            ...
            # ===== NEW: COMPUTE ODOM =====
            vL_mps = (lastL * 2*math.pi/60.0) * wheel_radius
            vR_mps = (lastR * 2*math.pi/60.0) * wheel_radius

            vx = (vR_mps + vL_mps) / 2.0
            wz = (vR_mps - vL_mps) / wheel_base

            now = time.time()
            dt_real = now - node.last_time
            node.last_time = now

            node.x += vx * math.cos(node.theta) * dt_real
            node.y += vx * math.sin(node.theta) * dt_real
            node.theta += wz * dt_real
            ...
            t.transform.translation.x = node.x
            t.transform.translation.y = node.y
            t.transform.rotation.z = math.sin(node.theta/2)
            t.transform.rotation.w = math.cos(node.theta/2)

            node.tf_broadcaster.sendTransform(t)

            print(f"L={rpmL_cmd:.1f} | R={rpmR_cmd:.1f} | vx={vx:.2f}")
